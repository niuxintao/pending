...
For example,  the 2-degree schema \{(Highlight, Off), (Status Bar, On)\} is a sub-schema (also is a real-sub-schema) of the 3-degree schema \{(Highlight, Off), (Status Bar, On), (Smart tags, Off)\}.
%Also, since a test case itself is a schema, then if a test case $t$ contains a schema $s$, we have $s \preceq t$. For example,  the 2-degree schema \{(Highlight, Off), (Status Bar, On)\} is also a sub-schema of a test case \{(Highlight, Off), (Status Bar, On), ( Bookmarks, On), (Smart tags, Off)\}.
...
This proposition shows that for two distinct schemas, one cannot be both super-schema and sub-schema of the other schema. For example, the 2-degree schema \{(Highlight, Off), (Status Bar, On)\} is a sub-schema of the 3-degree schema \{(Highlight, Off), (Status Bar, On), (Smart tags, Off)\}, but not a super-schema of this 3-degree schema.  Only two equal schemas can be both the sub-schema and super-schema of each other. For example, the 2-degree schema \{(Highlight, Off), (Status Bar, On)\} is both the sub-schema and super-schema of itself.
...
Note that test case is also a $n$-degree schema, hence when a test case $t$ hits one schema $s$, we can denote this relationship simply as $s \preceq t, |t| = n$. As an example, test case \{(Highlight, Off), (Status Bar, On), (Bookmarks, Off), (Smart tags, On)\} hits the 2-degree schema \{(Highlight, Off), (Status Bar, On)\}.
...
Formally,$\mathcal{T}(s) = \{ t | s \preceq t, |t| = n\}$. As an example, $\mathcal{T}($\{(Highlight, Off), (Status Bar, On)\}$)$ are the set of four test cases \{ \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, Off)\} ,  \{(Highlight, Off), (Status Bar, On), (Bookmarks, Off), (Smart tags, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, Off), (Smart tags, Off)\}  \}.
...
Formally,$\mathcal{I}(t) = \{ s | s \preceq t \}$. As an example, $\mathcal{I}$ ( \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\}\ ) is a set of $2^{4} - 1 = 15$ schemas, which is \{ \{(Highlight, Off)\}, \{(Status Bar, Off)\},  \{(Bookmarks, Off)\}, \{(Smart tags, Off)\},  \{(Highlight, Off), (Status Bar, Off)\}, ... , \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\} \}.
...
As an example, let test set $T$ be \{ \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, Off)\} \}. $\mathcal{S}(T)$ = \{ \{(Highlight, Off), (Status Bar, On), (Bookmarks, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, Off)\} \}. Other hit schemas of $T$ can appear in other test cases besides those in $T$. For example, \{(Highlight, Off), (Status Bar, On)\} can appear in the test case \{(Highlight, Off), (Status Bar, On), (Bookmarks, Off), (Smart tags, Off)\}, which is not in $T$.
...
Formally, $\mathcal{C}(T)  =  \{s | s \in \mathcal{S}(T)\ and\ \not\exists s' \prec s, s.t., s' \in \mathcal{S}(T)\}$. As an example, let test set $T$ still be \{ \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, On)\}, \{(Highlight, Off), (Status Bar, On), (Bookmarks, On), (Smart tags, Off)\} \}. $\mathcal{C}(T)$ = \{ \{(Highlight, Off), (Status Bar, On), (Bookmarks, On)\} \}. Note that other two schemas in $\mathcal{S}(T)$ are the super-schemas of this schema, therefore, they are eliminated from this set.
...
We have the following relationship between  minimal special schema set and test case set $T$.
...
We consider any abnormally executing a test case as a \emph{failure}. It can be triggered by a thrown exception, compilation error, assertion failure or constraint violation. Such a test case is called a \emph{failing} case. Otherwise, a test case is a \emph{passing} test case if the execution normally works without triggering any failure. In this paper, we only discuss the failure that are triggered by schemas.

To facilitate our discussion, we introduce the following two assumptions that will be used throughout this paper:
...
A schema is \emph{healthy schema} if and only if at least one passing test case containing this schema.
 %$\exists t \in \mathcal{T}(c)$, $t$ is a passing test case.% In addition,
\end{definition}
...
%if we can find at least one passing test case that contains this schema. 

\begin{definition}\label{de:faulty}
A schema is \emph{faulty schema} if and only if any test case containing this schema is a failing test case.
\end{definition}
...
\begin{proposition}[Minimal failure-causing schemas and failing test set]\label{pro:faultyschemastests}
