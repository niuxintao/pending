...
\begin{proposition}[Determinable with full set]\label{pro:determinablefull}
...
\begin{proposition}[Determinable to be faulty with part set]\label{pro:determinablepartfaulty}
...
Hence a set of schemas is healthy-complete is all the schemas should be in this set.
...
\begin{proposition}[Determinable to be healthy with schemas]\label{pro:determinableschemeashealthy}
Given the results of part of schemas $T$, $s$ is determinable to be healthy, if and only if $s \in T$.
\end{proposition}



\begin{proposition}[Determinable to be faulty with schemas]\label{pro:determinableschemeasfaulty}
...
\begin{proposition}[Determinable to be healthy with $HSS^{\top}$ ]\label{pro:determinableschemeashealthymax}
...
\begin{proposition}[Determinable to be faulty with  $FSS^{\bot}$]\label{pro:determinableschemeasfaultymin}
...
Given Two sets, is determinable, either satifies. Otherwise it is did not satifies.


\subsection{What is pending schema}

Through these motivating examples, we can learn that the pending schema is not checked to be healthy and faulty. Hence, we give the definition of the pending schema as follows. That is it does not fulfil the determinable properties. 

Not fulfil Determinable to be healthy with $HSS^{\top}$, and not fulfil  Determinable to be healthy with $FSS^{\bot}$
