...
To depicts Proposition \ref{pro:healthysintequal2} more clearly, we next check this proposition in the condition that $T_{pass}^{t\bigtriangleup} \neq T_{pass}^{t}$. Since in the previous examples, for any test case $t$ in the SUT,  $T_{pass}^{t\bigtriangleup}$ always equals to $T_{pass}^{t}$ (the two test cases in $T_{pass}$ are two disjoint schemas), we need to consider other test sets instead of $T_{pass}$ to check this proposition. We let $T_{temp} =$ \{$ t_{1} , t_{2} $\}, where $t_{1}$ = \{$(p_{1}, 0)$, $(p_{2}, 0)$, $(p_{3}, 1)$, $(p_{4}, 1)$ \}, and $t_{2}$ = \{$(p_{1}, 0)$, $(p_{2}, 1)$, $(p_{3}, 1)$, $(p_{4}, 1)$ \}, as shown in Table \ref{ex:determinableschemassut}.  Next, we still focus the failing test case $t_{7}$, and we have $T_{temp}^{t_{7}}$ = \{ \{$(p_{3}, 1)$, $(p_{4}, 1)$ \}, \{$(p_{2}, 1)$, $(p_{3}, 1)$, $(p_{4}, 1)$ \} \}, while $T_{temp}^{t_{7}\bigtriangleup}$ = \{ \{$(p_{2}, 1)$, $(p_{3}, 1)$, $(p_{4}, 1)$ \} \}. These two schema sets are not equal. Table \ref{ex:maximalhealthyintequal2} shows the schemas of $\{ s\ |\ s \preceq t_{7}\ \&\&\ \forall m \in T_{temp}^{t_{7}}, s \npreceq m \}$ and $\{ s\ |\ s \preceq t_{7}\ \&\&\ \forall m \in T_{temp}^{t_{7}\bigtriangleup}, s \npreceq m \}$, we find these two schema sets are still equal.

 \begin{table}[htbp]
  \centering
  \caption{An example of the reduction to maximal schemas}
  \label{ex:maximalhealthyintequal2}
    \begin{tabular}{|c|c|} \hline
     \multicolumn{2}{|c|}{$\{ s\ |\ s \preceq t_{7}\ \&\&\ \forall m \in T_{temp}^{t_{7}}, s \npreceq m \} $} \\ \hline
   $s_{31}$ & \{($p_{1}, 1$)\} \\
   $s_{51}$ & \{($p_{1}, 1$),($p_{2}, 1$)\} \\
   $s_{56}$ & \{($p_{1}, 1$),($p_{3}, 1$)\} \\
   $s_{32}$ & \{($p_{1}, 1$),($p_{4}, 1$)\} \\
...
\quad
    \begin{tabular}{|c|c|} \hline
   \multicolumn{2}{|c|}{$\{ s\ |\ s \preceq t_{7}\ \&\&\ \forall m \in T_{temp}^{t_{7}\bigtriangleup}, s \npreceq m \}$}\\ \hline
   $s_{31}$ & \{($p_{1}, 1$)\} \\
   $s_{51}$ & \{($p_{1}, 1$),($p_{2}, 1$)\} \\
   $s_{56}$ & \{($p_{1}, 1$),($p_{3}, 1$)\} \\
   $s_{32}$ & \{($p_{1}, 1$),($p_{4}, 1$)\} \\
