...
In Figure \ref{fig_finovlp_multi}, test cases $t_{1}$ to $t_{9}$ are the same that of the example in Figure \ref{fig_fic_multi}. We can observe that FINOLP first generated the test case $t_{10}$ to check whether there exist other MFS than the identified (-, -, 1, 1). Since $t_{10}$ failed after testing, it repeated the FIC on $t_{10}$ to identified the remaining MFS. Hence, it genertated four additional test cases $t_{11}$ to $t_{14}$. The passings of $t_{11}$ and $t_{12}$ indicated another MFS is (1, 0, -, -). 

FINOLP also checked whether there existed any other MFS by generating test case $t_{15}$. The passing of $t_{15}$ shows that there does not exist any other MFS in the original failing test case $t_{5}$. Above all, FINOLP identified all the MFS accurately as we have set in piror.

Next, the same as before, we use the two rules to check the status of the pending schemas in the original failing test case. The result is shown in the ``\emph{Status}'' of Figure \ref{fig_finovlp_multi}. We can observe that there still exists four schemas, which are (1, -, 1, -), (1, -, -, 1), (-, 0, 1, -), and (-, 0, -, 1), respectively. It is easy to find that these four schemas neither contain any MFS nor appears in any passing test cases. 


At last, we can learn that although FIC and FINOLP can handle the multiple MFS problem, it is still incomplete for the existence of these pending schemas. 

\subsection{Additional efforts to remove the pending schemas}

Since, FINOLP still cannot remove all the pending schemas. We 
...
Since there the approaches exist pending schemas, especally in the multiple MFS. and it is very hard to obtain them. We need to obtain new appraoch to formulate it and obtain them more efficiently. It needs a formal definiton and therotical analysis for the pending schemas with non-faulty schemas and faulty schemas.
