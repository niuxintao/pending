...
\begin{proposition}[pending schemas with passing test cases and minimal faulty schemas]\label{pro:pendingparthealthyfaulty}
...
In the previous section, we have already obtained the formula to describe the set of pending schemas for given information. However, this set is usually too large and many schemas in this set are beyond the focus. For example, the schema that never appears in any generated test case is evidently pending schema, but we didn't even cover them, these schemas are the task of test generation. As an alternative, it is more important to focus on these schemas that have already appeared in these generated test cases. This is because after testing and MFS idnetificaiton, we may have a false condendce that we have already known all the schemas, but the fact is the extences of the pending schemas in these test cases, the testing and MFS identiicaiton is still not complete. Consequently, we only focus the pending schemas that appeared in the geenrated test cases.
...
More formally,  given a failing test case $t$ =\{$(p_{1}, v_{1})$, $(p_{2}, v_{2})$, ..., $(p_{n}, v_{n})$\},  a set of passing test cases $PSS$ = \{ $t_{1}, t_{2}, ... t_{i}, ...$\}, and a set of minimal faulty schemas $MFSS$ = \{ $m_{1}, m_{2}, ... m_{i}, ...$ \}, the goal is to find the set of pending schemas in test case $t$.
...
PSS=\{ c | c \preceq t\ \&\&\ \forall c_{f} \in MFSS,  c_{f} \npreceq c  \ \&\&\  \forall c_{h} \in DSS, c \npreceq c_{h} \} .
\end{aligned}
\end{equation}
