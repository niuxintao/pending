...
In this subsection, we will formally define the test case and schema. We will also give the properties to reveal the relationships between them. In these propoertations, Propoertin   are origined from our previous work .
...
Note that test case is also a $n$-degree schema, hence when a test case $t$ hits one schema $s$, we call denote this relationship simply as $s \preceq t, |t| = n$. As an example, test case \{(Highlight, Off), (Status Bar, On), (Bookmarks, Off), (Smart tags, On)\} hits the 2-degree schema \{(Highlight, Off), (Status Bar, On)\}.
...
With respect to the minimal special schema set of test case $T$, we can obtain the following property.

\begin{proposition}[Property of minimal special schema set of test Set T]\label{pro:ash}
For any test set $T$ of the SUT, $\bigcup_{s \in \mathcal{C}(T)} \mathcal{T}(s) = T$.
\end{proposition}

\begin{proof}
As $\mathcal{C}(T) = \{s | s \in \mathcal{S}(T)\ and\ \not\exists s' \prec s, s.t., s' \in \mathcal{S}(T)\}$, indicating that $\mathcal{C}(T) \subseteq \mathcal{S}(T)$.  It is then obviously $\bigcup_{s \in \mathcal{C}(T)} \mathcal{T}(s) \subseteq \bigcup_{s \in \mathcal{S}(T)} \mathcal{T}(s)$. Hence, we just need to prove that $\bigcup_{s \in \mathcal{S}(T)} \mathcal{T}(s) \subseteq \bigcup_{s \in \mathcal{C}(T)} \mathcal{T}(s)$.
...
\begin{proposition} [Minimal schema of the subset of test set $T$]\label{pro:sbS}
 For any test set $T$ and schema $s$ of the  SUT, if $\mathcal{T}(c) \subseteq T$, then  $c \in \mathcal{S}(T) $.
\end{proposition}
\begin{proof}
...
A schema, say, $c$, is \emph{healthy schema} if and only if $\exists t \in \mathcal{T}(c)$, $t$ is a passing test case.% In addition, if none of super-schema of $c$ except itself is the \emph{healthy schema}, we then call the schema $c$ the \emph{maximal healthy schema (MHS)}.
\end{definition}
...
\begin{proposition}[Faulty schema]\label{pro:subofhealthy}
