...
Let $PTS^{t}$ be the schemas that are both the subschemas of test case $t$ and the subschemas of at least one schema in $PTS$. That is, $PTS^{t}$ = $\{ s\ | \exists s' \in PTS, s \preceq s' \&\& s \preceq t \}$. Note that to obtain $PTS^{t}$ from $PTS$, the complexity is $O(|PTS|)$.
 
 Let $PTS^{t\bigtriangleup}$ be the maximal schemas that are both the subschemas of test case $t$ and the subschemas of at least one schema in $PTS$. That is, $PTS^{t\bigtriangleup}$ = $\{ s\ |  s \in PTS^{t} \&\& \nexists s'  \in PTS^{t}, s \prec s' \}$.
 
\begin{equation}
\begin{aligned}\label{eq:pssfirst_mutated}
PSS=\{ s | s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq c  \ \&\&\  \forall p \in PTS^{t\bigtriangleup}, s \npreceq p \} .
\end{aligned}
\end{equation}
