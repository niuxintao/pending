...
Next, let us use OFOT to identify the MFS and help to  narrow down the set of pending schemas of $t_{5}$ in the original covering array.  To make OFOT useful, let us first assume that there is one MFS (-, -, 1, 1) in failing test case $t_{5}$. 
Then the OFOT will generate four additional test cases $t_{6}$ to $t_{9}$ as shown in Figure \ref{fig_ofot_single}, respectively, with one factor have been mutated from $t_{5}$ at each time.  Since, $t_{8}$ and $t_{9}$ failed, then (-, -, 1, -) and (-, -, -, 1) are two factors in the MFS according to the MFS. The passing of $t_{6}$ and $t_{7}$ shows there is no other factors in this MFS. Hence, OFOT identified the Schema (-, -, 1, 1) as the MFS, which is idneitifical to our setting in prior.
...
To analyse the pending schemas, we also list all the schemas in each test case as we did before. 
...
\subsection{The incompleteness of Finovlp}
