...
Table \ref{ex:healthyintequal} shows these two schema sets, i.e., $\{ s\ |\ s \preceq t_{2}\ \&\&\  \forall t_{i} \in T_{pass}, s \npreceq t_{i} \} $ and $\{ s\ |\ s \preceq t_{2}\ \&\&\ \forall p \in T_{pass}^{t_{2}}, s \npreceq p \}$. We can learn these two sets are equal.
...
\quad
    \begin{tabular}{|c|c|} \hline
   \multicolumn{2}{|c|}{$\{ s\ |\ s \preceq t_{2}\ \&\&\ \forall p \in T_{pass}^{t_{2}}, s \npreceq p \}$}\\ \hline
   $s_{28}$ & \{($p_{3}, 1$)\} \\
   $s_{62}$ & \{($p_{1}, 0$),($p_{2}, 0$)\} \\
   $s_{30}$ & \{($p_{1}, 0$),($p_{3}, 1$)\} \\
   $s_{32}$ & \{($p_{2}, 0$),($p_{3}, 1$)\} \\
...
With this proposition, we do not need to check each schema in $T_{pass}^{t}$, instead, we only need to focus on the maximal schemas in this set.   Going back to the previous example, we have $T_{pass}^{t_{2}\bigtriangleup}$ = \{ \{($p_{1}, 0$) , ($p_{4}, 1$)\}, \{($p_{2}, 0$), ($p_{4}, 1$)\} \}. Table \ref{ex:maximalhealthyintequal} shows the schemas of $\{ s\ |\ s \preceq t_{2}\ \&\&\ \forall p \in T_{pass}^{t_{2}\bigtriangleup}, s \npreceq p \}$, we can observe this set is equal to the previous schema sets, that are  $\{ s\ |\ s \preceq t_{2}\ \&\&\  \forall t_{i} \in T_{pass}, s \npreceq t_{i} \} $ and $\{ s\ |\ s \preceq t_{2}\ \&\&\ \forall p \in T_{pass}^{t_{2}}, s \npreceq p \}$, respectively.
 ...
 \begin{table}[htbp]
  ...
  \label{ex:maximalhealthyintequal}
    \begin{tabular}{|c|c|} \hline
     \multicolumn{2}{|c|}{$\{ s\ |\ s \preceq t_{2}\ \&\&\ \forall p \in T_{pass}^{t_{2}\bigtriangleup}, s \npreceq p \}$} \\ \hline
   $s_{28}$ & \{($p_{3}, 1$)\} \\
   $s_{62}$ & \{($p_{1}, 0$),($p_{2}, 0$)\} \\
   $s_{30}$ & \{($p_{1}, 0$),($p_{3}, 1$)\} \\
   $s_{32}$ & \{($p_{2}, 0$),($p_{3}, 1$)\} \\
