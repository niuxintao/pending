...
As an example, we consider the same  $T_{fail}$, $T_{pass}$, and $T_{unknown}$ as in Table \ref{ex:determinableschemas}. The pending schemas for this example are listed in Table \ref{ex:pendingschemas}. We can learn that each schema in Table \ref{ex:pendingschemas} is neither contained in set $\mathcal{I}(T_{pass})$ nor contained in set $\mathcal{S}(T_{fail})$ of Table \ref{ex:determinableschemas}.
