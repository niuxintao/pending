...
\begin{proof}
Let us first prove the sufficiency. Let $s_{1}$ is a faulty schema. According to the definition, $\forall t$, $s_{1} \preceq t$ and $t$ is a failing test case. In fact, $t$ itself is a schema, and $t \preceq t$, which indicating that $t$ is a faulty schema. Hence, the statement that $\exists s_{2}, s_{1} \preceq s_{2}$ $\&\&$ $s_{2}$ is a healthy schema is true, because $t$ is such a schema.
...
\begin{proposition}[Undetermined to be Faulty]\label{pro:nonsuperoffaulty}
...
first give the formal defition of not be detemined to be healthy.
then give the formal definition of not be determiend ot faulty.
