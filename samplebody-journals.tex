...
\begin{proof}
According to the definition of healthy schema,  there exists at least one test case of the whole set of containers  of $s_{1}$  passes. Let this test case be $t$. Obviously, $s_{1} \preceq t$.  Then $\forall s_{2} \preceq  s_{1}$, we have $s_{2} \preceq s_{1} \preceq t$ according to Proposition \ref{pro:subsumetrans}. That is, the passing test case $t$ is also one of the containers of $s_{2}$. According to the definition of healthy schema, $\forall s_{2} \preceq  s_{1}$, $s_{2}$ is a healthy schema.
\end{proof}
...
Next, let us discuss the relationships among tests, faulty schemas, and healthy schemas. We let the set of failing test cases in the SUT be $T_{fail}$, and the passing test cases in the SUT be $T_{pass}$. Obviously, $T_{pass} = \overline{T_{failing}}$. We can further conclude the following propositions.
...
Proposition \ref{pro:healthyschemastests} tells 
