...
The first step is to reduce the space of checking faulty schemas and passing test cases. Let $MFSS_{t}$ be the minimal faulty schemas that are the subschemas of test case $t$. That is, $MFSS^{t}$ = $\{ s\ | \ s \in MFSS, s \preceq t \}$. Let $MFSS^{t\circ}$ = $MFSS \backslash MFSS^{t}$. It is obvious that the complexity of obtaining $MFSS^{t}$ from $MFSS$ is $O(|MFSS|)$.
...
A = $\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS,  m \npreceq s \}$
 =
$\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \&\&\   \forall m \in MFSS^{\circ},  m \npreceq s  \}$

Since $MFSS^{t\circ}$ = $MFSS \backslash MFSS^{t}$ = $\{ m \ | \ m \in MFSS, m \npreceq t \}$.
Hence, $\forall s \preceq t, m \in MFSS^{t\circ},  m \npreceq s$ (Otherwise, $m \preceq s \preceq t$, which is contradiction).

Since the condition $\forall s \preceq t, \forall m \in MFSS^{\circ},  m \npreceq s $ always hold, hence,  A =
$\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \&\&\   \forall m \in MFSS^{\circ},  m \npreceq s  \}$  =
$\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \}$ = B. 
\end{proof}
...
%\begin{proof}
%
%Let A = $\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS,  m \npreceq s \}$, B = $ \{ s\ |\ s \preceq t\ \&\&  \forall t_{i} \in PTS, s \npreceq t_{i} \} $. Obviously, PSS = A $\cap$ B.
%
%A = $\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS,  m \npreceq s \}$
% =
%$\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \&\&\   \forall m \in MFSS^{\circ},  m \npreceq s  \}$
...
%$\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \}$
%
%Next, we will prove the set B equals to the set B' = $ \{ s\ |\ s \preceq t\ \&\&  \forall t_{i} \in  PTS^{t\bigtriangleup}, s \npreceq t_{i} \} $. Note that $PTS^{t}$ = $\{ s\ | t' \cap t, t' \in PTS \}$, and we have $\forall s \in PTS^{t}, s \preceq t \&\& \exists t' \in PTS, s \preceq t'$. 
%
%We first prove that $B \subseteq B'$. Note for each $s \in B$, we have $s \preceq t \&\& \forall t_{i} \in PTS, s \npreceq t_{i}$. We assume that $\exists s \in B, \exists s' \in PTS^{t}, s \preceq s'$. For $s'$, $\exists t' \in PTS, s' \preceq t'$. Hence, $s  \preceq s' \preceq t'$. This is contradiction. Hence the assumption does not hold, indicating that $\forall s \in B, \forall s' \in PTS^{t}, s \npreceq s' \&\& s \preceq t$. That is $\forall s \in B$, $s \in B'$. Hence, $B \subseteq B'$.
%
%Next we prove that $B' \subseteq B$. Since $\forall s \in B'$, we have $s \preceq t \&\& \forall t_{i} \in  PTS^{t}, s \npreceq t_{i}$. We assume that $\exists s \in B', \exists t' \in PTS, s \preceq t'$. Also $s \preceq t$, which indicates that $s \preceq (t \cap t')$.  Since $(t \cap t') \in PTS^{t} $, $\exists s \in B', \exists t_{i} \in PTS^{t}, s \preceq t_{i}$. This is contradiction, indicating that  $\forall s \in B', \forall t' \in PTS, s \npreceq t' \&\& s \preceq t$. That is, $\forall s \in B'$, $s \in B$. Hence, $B' \subseteq B$.
%
%Since we have $B \subseteq B'$ and $B' \subseteq B$, we have $B = B'$.
%
%At last, PSS = $A \cap B$ = $\{ s\ |\  s \preceq t\ \&\&\ \forall m \in MFSS^{t},  m \npreceq s \}$ $\cap$  $ \{ s\ |\ s \preceq t\ \&\&  \forall t_{i} \in  PTS^{t\bigtriangleup}, s \npreceq t_{i} \} $ =
% $ \{ s\ |\ s \preceq t\ \&\&  \forall m \in MFSS^{t},  m \npreceq s  \ \&\&\  \forall t_{i} \in  PTS^{t\bigtriangleup}, s \npreceq t_{i} \} $
%
%\end{proof}
...
Let $t$ be the failing test case, the passing test case set $PTS$ be the set of schemas $HSS$, the minimal faulty schemas $MFSS$ be the set of schemas $FSS$. Then we transform the aforementioned formula (Formula \ref{eq:pssfirst}) to be the following equation.
