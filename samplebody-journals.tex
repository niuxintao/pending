...
Here, the ``information'' indicates the outcomes of the execution of some test cases or the determination state of existed schemas. Note that if given full set of test cases, we can always determine which schema is faulty and which is healthy.

However, if only partial information, this is not so.

Next we discuss two conditions under which schemas are determinable.

First, given part of passing test cases.

Second, given a set of faulty schemas, of which satisfying that no other schemas of its faulty is faulty schema (This is the definition of MFS identification).
...
\begin{proposition}[Pending o be healthy with part set]\label{pro:determinableparthealthy}
