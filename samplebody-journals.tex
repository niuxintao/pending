...
\subsection{Determinable schemas}\label{sec:pending:determinable}
Before we give a formal definition of pending schema, we need to first give the definition of ``a determinable schema". Note that in Section \ref{sec:back}, we introduced the definition of faulty schema and healthy schema, as well as some properties of them. Based on these definitions, we can conclude that in order to determine a schemas to be healthy or faulty, we need to make sure whether the corresponding schema satisfies the specific conditions. Hence, if these conditions are given at first or can be deduced from existed information, we can determine the the schema to be healthy or faulty. Otherwise, we cannot determine the state of the schema for lack of information. Next, we will give the definition of the determinable schema.
...
Here, the ``information'' indicates the outcomes of the execution of some test cases or the determination state of existed schemas. If given the outcomes of all the test cases in the SUT or the determination of all the possible schemas in the SUT, then for any schema in the SUT, we can easily determine it to be faulty or healthy. However, if only given partial information, it is obvious that not all the schemas are determinable.

Next, we will discuss the determination of schemas under two particular circumstances. These two circumstances are important to analyse the properties of schemas for MFS identification later.
...
Let us first assume that $s$ is determined to be a heathy schema. Then according to the definition, $\exists t, s \preceq t$, $t$ is a passing test case. Since, it does not have to be passing test cases in this condition (in fact, all the failing test cases possible to be). Hence, $s$ cannot to be a healthy schema.

Then we assume that $s$ is determined to be a faulty schema. We need to prove it has $\exists m \in MFSS, m \preceq s$.
We also prove by contradiction. We assume that $\nexists m \in MFSS, m \preceq s$. There are two conditions. First, $\exists m \in MFSS, s \preceq m$. This is not possible, as all the schemas are minimal. Second $\forall m \in MFSS$, $s \npreceq m$ and $m \npreceq s$. This is also not possible (We prove it in the Appendix). Since, these two conditions are all not possible. Hence, $\exists m \in MFSS, m \preceq s$. 
...
\begin{proposition}[Determinable with passing test cases and MFS]\label{pro:determinableparthealthyfaulty}
Given a set of passing test case set $PTS$, and a set of minimal faulty schemas $MFSS$, $\forall s$ is determinable if only if either $\exists t \in PTS, s \preceq t$ or $\exists m \in MFSS, m \preceq s$.
\end{proposition}
...
\subsection{What is pending schema}\label{sec:pending:pending}
...
Next, we prove that this proposition is hold.

\begin{proposition}[pending schema]\label{pro:pendingFromtwoSets}
Given two set of schemas $HSS$, $FSS$, and $\forall s_{1} \npreceq HSS$ and $forall FSS \npreceq s_{1}$, then $s_{1}$ is a pending schema.
\end{proposition}
...
\subsection{Obtaining the pending schemas in a failing test case}\label{sec:pending:obtaining}


To given a set of healthy schemas from passing test case. Given a set of MFS, and the super-schemas of these MFS. Then the quesion is to find the pending schemas.

Note that any sub-schemas of the MFS is not the faulty schemas, at least by the definition. Hence, it should not be the super-schemas of the MFS. Hence, as all the healthy schemas are appeared in the passing test cases. Hence, we can learn all the healthy schemas. This is also true by definition.

Hence, the future work is .
...
\begin{proposition}[No to be faulty schema]\label{pro:subofCMXS}
