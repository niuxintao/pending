...
We conduct a series studies on some real software systems with number ranged from small to large. There are two studies we conducted, first, we first studies the incompleteness of different fault localization approaches in CT and covering arrays. Second, we analyse the efficiency of three formulas. Our results mainly shows that the incompleteness in a common in the MFS identification approaches, especially in those efficient MFS approaches. We also empirically observed that the Formula 3 is the most efficient formula among others to obtain the pending schemas.
