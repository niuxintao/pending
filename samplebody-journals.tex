...
\end{figure*}
 ...
 As a result, since there is no passing test cases, no more schemas is labeled as non-faulty. And since there is no more MFS, there is no more schemas will be identified as faulty schemas. As a result, the stats of $t_{5}$ will not change, which is the same as the original covering array. Hence, we can learn that in the multiple MFS condition, the MFS identiicaiotn appraoch OFOT is still incomplete.
   

\subsection{The incompleteness of FIC}

For the example of OFOT, we can learn the multiple MFS in one failing test case is the main cause of incompeletness. FIC, an mutated verison of OFOT is proposed to handle the problem. It also muate the one factor at one time. But different from the , it keeps the change of original . To do this enables the FIC get away from the multiple MFS influence.
% and FIC\_BS
...
Since there the approaches exist pending schemas, especally in the multiple MFS. and it is very hard to obtain them. We need to obtain new appraoch to formulate it and obtain them more efficiently.
